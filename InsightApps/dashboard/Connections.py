...
def sheet_ds_delete(dsdt,sheet_id,server_id,queryset_id):
    ...
    for i2 in dsdt:
        ...
        models.dashboard_data.objects.filter(sheet_ids=o_shid,selected_sheet_ids=sl_shid).update(sheet_ids=a1,selected_sheet_ids=sl,datasrc=filesave['file_url'],datapath=filesave['file_key'])
    ...
    if parameter=="save":
        ...
        if models.dashboard_data.objects.filter(user_id=u_id,id=ds_id).exists() or models.dashboard_data.objects.filter(id=ds_id,user_ids__contains=u_id):# and u_id in a:
            ...
            if old.dashboard_name==dashboard_name:
                ...
                models.dashboard_data.objects.filter(id=ds_id,dashboard_name=dashboard_name).update(role_ids=role_ids,user_ids=user_ids,file_id=file_id,selected_sheet_ids=selected_sheet_ids,
                ...
                height=height,width=width,grid_id=gr_tb.id,queryset_id=queryset_id,server_id=server_id)
                return Response({'message':'Updated Successfully'},status=status.HTTP_200_OK)
            else:
                if models.dashboard_data.objects.filter(user_id=u_id,dashboard_name=dashboard_name).exists() or models.dashboard_data.objects.filter(dashboard_name=dashboard_name,user_ids__contains=u_id):# and u_id in a:
                    return Response({'message':'Dashboard Name already exists, please rename the Dashboard name'},status=status.HTTP_406_NOT_ACCEPTABLE)
                    ...
                    sheet_ids=sheet_ids,datapath=file_sv["file_key"],datasrc=file_sv["file_url"],updated_at=datetime.datetime.now(),dashboard_tag_name=dashboard_tag_name,
                    role_ids=role_ids,user_ids=user_ids,height=height,width=width,grid_id=gr_tb.id,queryset_id=queryset_id,server_id=server_id)
                    return Response({'message':'Updated Successfully'},status=status.HTTP_200_OK)
        else:
...
def charts_dt(charts,tok1,server_id,file_id):
    ...
    cl_list=[]
    for ch in charts:
        if ch['server_id']==None or ch['server_id']=="" or ch['server_id']=='':
            ser_dt=models.FileDetails.objects.get(id=ch['file_id'])
        else:
            ser_dt=models.ServerDetails.objects.get(id=ch['server_id'])
...
def dashboard_dt(charts,charts_user,tok1):
    ...
    for ch in charts:
        # print(ch['id'])
        # if ch['sheet_ids']==[] or ch['sheet_ids']==None or ch['sheet_ids']=='':
        #     sheets_data=[]
        # else:
        #     shet_ids=litera_eval(ch['sheet_ids'])
        #     for sh in shet_ids:
...
def query_sheet_search(qrse,tok1,sheet_ids):
    ...
    for qr in qrse:
        if qr['server_id']==None or qr['server_id']=='' or qr['server_id']=="":
            serdt=models.FileDetails.objects.get(id=qr['file_id'])
            created=serdt.uploaded_at.date()
            updated=serdt.updated_at.date()
            file_id=serdt.id
            serber_id=None
        elif qr['file_id']==None or qr['file_id']=='' or qr['file_id']=="":
            serdt=models.ServerDetails.objects.get(id=qr['server_id'])
        ...
        sheet_data['database_name']=serdt.display_name
        sheet_data['created_by']=tok1['username']
        ...
        if tok1['status']==200:
            ...
            if serializer.is_valid(raise_exception=True):
                ...
                if search=='' or search==None:
                    qrse=models.QuerySets.objects.filter(user_id=tok1['user_id']).values().order_by('-updated_at')
                    ...
                    for qrs in querysets_li:
                        qrse=models.QuerySets.objects.filter(queryset_id=qrs).values().order_by('-updated_at')
            ...
            if serializer.is_valid(raise_exception=True):
                ...
                if models.dashboard_data.objects.filter(id=dashboard_id,user_id=tok1['user_id']).values().exists():
                    ...
                    if ex_sh_ids==[] or ex_sh_ids=="":
                        models.dashboard_data.objects.filter(id=dashboard_id,user_id=tok1['user_id']).update(selected_sheet_ids=sheet_ids)
                    else:
                        for shid in sheet_ids:
                            ex_sh_ids.append(shid)
                        models.dashboard_data.objects.filter(id=dashboard_id,user_id=tok1['user_id']).update(selected_sheet_ids=ex_sh_ids)
                    return Response({'message':'updated successfully'},status=status.HTTP_200_OK)
                else:
                    return Response({'message':'Dashboard not exists'},status=status.HTTP_404_NOT_FOUND)
            else:
...
def queryset_list(request,token):
    if request.method=='GET':
        tok1 = views.test_token(token)
        if tok1['status']==200:
            qrsets=models.QuerySets.objects.filter(user_id=tok1['user_id']).values().order_by('-updated_at')
            data = []
            for qr in qrsets:
                data.append({'id':qr['queryset_id'],'queryset_name':qr['query_name']})
            return Response(data,status=status.HTTP_200_OK)
        else:
            return Response(tok1,status=tok1['status'])
    else:
